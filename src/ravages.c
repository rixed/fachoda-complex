...
void explose(int oc, int i) {
	...
	subv(&p,&obj[0].pos);
	np=renorme(&p);
	playsound(VOICEEXTER,EXPLOZ,1+(randK()-.5)*.1,1./(1+np*np*1e-6),128*scalaire(&p,&obj[0].rot.x));
	copyv(&vit,&vec_zero);
	switch (obj[oc].type) {
	case CIBGRAT:
		o1=oc;
		...
		if (cmoi<NBBOT && kelkan(o1)!=bot[cmoi].camp) bot[cmoi].gold+=900*randK()*(o1==oc?3:1);	// les églises valent plus cher !
		break;
	case AVION:
		o1=oc;
		...
		if (v==visubot) {
			...
			playsound(VOICEGEAR,DEATH,1+(randK()-.5)*.15,1,0);
		} else randK();
		break;
	case VEHIC:
		o1=oc;
	...
	//		obj[j].model=nobjet[NBNAVIONS+NBBASES+NBMAISONS+NBVEHICS+6].firstpiece;
		...
		for (i=0, j=jk; i<NBGRAVMAX && j<o2; i++) {
			if (debris[i].o==-1) {
				...
				debris[i].a1=randK()*M_PI*2;
				debris[i].a2=randK()*M_PI*2;
		...
		if (j<NBFUMEESOURCE) {
			fumeesource[j]=o1;
			fumeesourceintens[j]=randK()*2000;
		}
...
void hitgun(int oc, int i) {
	...
	if (o1) {
		switch (obj[o1].type) {
		case CIBGRAT:
			if (cmoi<NBBOT && kelkan(o1)!=bot[cmoi].camp) if (randK()<.05) bot[cmoi].gold+=60;
			if (o1!=oc && randK()<.01) explose(o1,i);
			break;
		case AVION:
			for (j=0; j<NBBOT; j++) if (bot[j].vion==o1) {
				tarif=-(bot[j].fiulloss/4+bot[j].motorloss*8+bot[j].aeroloss*8+bot[j].bloodloss*2);
				if (j==bmanu) accel=0;
				if (j==visubot) playsound(VOICEGEAR,HIT,1+(randK()-.5)*.1,randK(),(randK()-.5)*127); else {randK(); randK(); randK();}
			//	printf("bot %d hit\n",j);
				if (randK()<.1) bot[j].fiulloss+=randK()*100;
				if (randK()<.04) if ((bot[j].motorloss+=randK()*10)<0) bot[j].motorloss=127;
				if (randK()<.05) if ((bot[j].aeroloss+=randK()*10)<0) bot[j].aeroloss=127;
				if (randK()<.04) {
					bot[j].bloodloss+=randK()*100;
					if (j==visubot) playsound(VOICEGEAR,PAIN,1+(randK()-.2)*.1,1,0); else randK();
				}
				if (randK()<bot[j].nbomb/1000. || randK()<.05) {
					bot[j].burning+=randK()*1000;
					if (bot[j].burning>900) explose(o1,i);
					bot[j].fiulloss+=randK()*200;
					if ((bot[j].motorloss+=randK()*100)<0) bot[j].motorloss=127;
					if ((bot[j].aeroloss+=randK()*100)<0) bot[j].aeroloss=127;
				}
